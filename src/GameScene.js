...
export default class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreText = this.scene.scene.add.text(
        16,
        16,
        `score: ${this.score}`,
  ...
  update(time, delta) {
    ...
    if (this.input.activePointer.isDown) {
      const rad = Math.atan2(
          Math.min(LOW_PLANE_BARRIER, this.input.activePointer.y) - this.physicsPlane.y,
          Math.max(this.physicsPlane.x, Math.abs(this.input.activePointer.x - this.physicsPlane.x)));
  ...
  manageSpeed(rad) {
    this.clouds.getChildren().forEach((el) =>
      el.setVelocityX(Math.min(Math.cos(rad) * 0.5 * el.baseVelocity, 0.7 * el.baseVelocity)));
  }
  ...
  respawnClouds() {
    if (this.time.now - this.lastTimeSpawnedCloud > this.CLOUD_SPAWN_TIME) {
      ...
      const y = Math.max(
          HIGH_CLOUD_BARRIER,
          Math.min(LOW_CLOUD_BARRIER, (Math.random() * (window.innerHeight + 50)) - 50)
      );

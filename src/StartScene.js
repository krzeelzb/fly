...
export default class Start extends Phaser.Scene {
    ...
    create(){
        ...
        const clickButton = this.add.text(window.innerWidth / 2 - 100, window.innerHeight/2 - 100, 'START', { fontSize: '64px', fill: '#fff', align: 'center' })
            .setInteractive()
        ...
        const welcomeText = this.add.text(window.innerWidth / 2 - 550, window.innerHeight/2 - 300, instructions, { fontSize: '20px', align: 'center', fill: '#fff' })
        const scoreText = this.add.text(window.innerWidth / 2 - 150, window.innerHeight/2 + 100, `SCORE: ${this.score === undefined ? 0 : this.score}`, { fontSize: '64px', align: 'center', fill: '#fff' })
    }
